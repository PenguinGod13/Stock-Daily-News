...
@patch("scanner.main.get_market_movers", return_value=[])
...
def test_run_sends_email_on_success(
    ...
    mock_get_movers, mock_get_news, mock_get_quote, mock_get_watchlist, mock_is_run_window,
):
...
def test_run_collects_ticker_errors_and_sends_debug_email(
    mock_send_debug, mock_send_email, mock_build_digest, mock_get_trends,
    mock_get_movers, mock_get_quote, mock_get_watchlist, mock_is_run_window,
):
    run(config=dict(BASE_CONFIG))
